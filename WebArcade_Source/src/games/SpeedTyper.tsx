...
export function SpeedTyper({ onBack }: Props) {
  ...
  const getDifficultySettings = (diff: Difficulty, currentLevel: number) => {
    // Distance to travel is ~110% (from -10% to 100%).
    // Speed = 110 / time_in_ms.
    // Beginner: 10 seconds (10000ms) = 110 / 10000 = 0.011
    let speed = 0.011; 
    let rate = 3000;
    let spawnCount = 1;

    if (diff === 'Intermediate') {
      // Intermediate: 7 seconds (7000ms) = 110 / 7000 = 0.0157
      speed = 0.0157;
      rate = 2000;
    } else if (diff === 'Expert') {
      // Expert: 5 seconds (5000ms) = 110 / 5000 = 0.022
      speed = 0.022;
      rate = 1500;
    }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (/[a-z]/.test(newChar) && newChar.length === 1) {
